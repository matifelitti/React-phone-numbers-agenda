...
function App() {
  ...
  const handleSave = (e) => {
    ...
    if (editingPhoneId !== null) {
      const updatedPhonesList = phonesList.map((phone) => {
        if (phone.id === editingPhoneId) {
          return {
            ...phone,
            name: newName,
            number: newNumber,
  ...
  const handleDelete = (phoneId) => {
    const updatedPhonesList = phonesList.filter(
      (phone) => phone.id !== phoneId
    );
    setPhonesList(updatedPhonesList);
  };

  const openEditModal = (phoneId) => {
    const phoneToEdit = phonesList.find((phone) => phone.id === phoneId);
    if (phoneToEdit) {
      setNewName(phoneToEdit.name);
      setNewNumber(phoneToEdit.number);
      setEditingPhoneId(phoneId);
      setModalOpenNewContact(true);
    }
  };
              ...
              <div className="modal-content">
                <div>
                  ...
                  <h5 className="modal-title">
                    {editingPhoneId !== null ? "Edit Contact" : "New Contact"}
                  </h5>
                </div>
                <div>
        ...
        <div className="scrollBar">
          {phonesList.map((phone) => (
            <div className="phonesData" key={phone.id}>
              ...
              <div className="icons">
                <button onClick={() => openEditModal(phone.id)}>
                  <svg
